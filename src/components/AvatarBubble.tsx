...
// so every screen shows the same character. Tapping it opens the Profile tab.
export default function AvatarBubble({ size = 50, onClick }: { size?: number; onClick?: () => void }) {
  return (
    <div
      className="community-avatar-bub"
      onClick={onClick}
      role={onClick ? 'button' : undefined}
      aria-label={onClick ? 'Open profile' : undefined}
      style={{ background: '#d6e475', width: size, height: size, borderRadius: size / 2, cursor: onClick ? 'pointer' : undefined }}
    >
